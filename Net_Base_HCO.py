...
class FrequencyValueEmbedding(nn.Module):
    ...
    def __init__(self, dim, num_heads=1):
        ...
        self.fve = nn.Parameter(torch.randn(1, dim, 1, 1))
    ...
    def forward(self, x_freq, h, w):
        ...
        # Use FVE to predict k
        fve_pooled = F.adaptive_avg_pool2d(x_freq + self.fve, (1, 1))  # (B, C, 1, 1)
    ...
    def initialize(self):
        nn.init.trunc_normal_(self.fve, std=0.02)
...
class HeatConductionOperator(nn.Module):
    ...
    def __init__(self, dim, num_heads=1, t=1.0):
        ...
        self.dct = DCT2D()
        self.idct = IDCT2D()
        self.fve = FrequencyValueEmbedding(dim, num_heads)
    ...
    def forward(self, x):
        ...
        # Predict thermal diffusivity k using FVEs
...
class HCOLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads=1, mlp_ratio=4.0, drop=0., t=1.0):
        ...
        # HCO branch
        self.hco = HeatConductionOperator(dim, num_heads, t)
...
##################################### Main Network with HCO Pathway (Option 2) ###################################################
class Net(nn.Module):
    def __init__(self, cfg ,backbone_path="./resnext/resnext_101_32x4d.pth"):
        ...
        # HCO Pathway: Process each ResNeXt stage output
        # Each HCO layer processes features from corresponding ResNeXt stage

        self.HCO_depth = [2, 2, 9, 2]
        #HCOLayer(dim=64, num_heads=1, mlp_ratio=4.0, t=1.0)
        self.hco_pathway = nn.ModuleList([
            nn.Sequential(*[HCOLayer(dim=64, num_heads=1, mlp_ratio=4.0, t=1.0) for _ in range(self.HCO_depth[0])]),
            nn.Sequential(*[HCOLayer(dim=256, num_heads=4, mlp_ratio=4.0, t=1.0) for _ in range(self.HCO_depth[1])]),  # for stage2 (256 channels)
            nn.Sequential(*[HCOLayer(dim=512, num_heads=8, mlp_ratio=4.0, t=1.0) for _ in range(self.HCO_depth[2])]),  # for stage3 (512 channels)
            nn.Sequential(*[HCOLayer(dim=1024, num_heads=16, mlp_ratio=4.0, t=1.0) for _ in range(self.HCO_depth[3])]),  # for stage4 (1024 channels)
            HCOLayer(dim=2048, num_heads=32, mlp_ratio=4.0, t=1.0)  # for stage5 (2048 channels)
        ])


        # self.hco_pathway = nn.ModuleList([
        #     HCOLayer(dim=64, num_heads=1, mlp_ratio=4.0, t=1.0),  # for stage1 (64 channels)
        #     HCOLayer(dim=256, num_heads=4, mlp_ratio=4.0, t=1.0),  # for stage2 (256 channels)
        #     HCOLayer(dim=512, num_heads=8, mlp_ratio=4.0, t=1.0),  # for stage3 (512 channels)
        #     HCOLayer(dim=1024, num_heads=16, mlp_ratio=4.0, t=1.0),  # for stage4 (1024 channels)
        #     HCOLayer(dim=2048, num_heads=32, mlp_ratio=4.0, t=1.0)  # for stage5 (2048 channels)
        # ])

        # Channel alignment layers for HCO pathway connections
        self.hco_align = nn.ModuleList([
            nn.Conv2d(64, 256, 1),  # align stage1 HCO output to stage2
            nn.Conv2d(256, 512, 1),  # align stage2 HCO output to stage3
            nn.Conv2d(512, 1024, 1),  # align stage3 HCO output to stage4
            nn.Conv2d(1024, 2048, 1),  # align stage4 HCO output to stage5
        ])


        # Replace Pyramid Pooling (GE) with projection from final HCO output
        # self.pyramid_pooling = PyramidPooling(2048, 64)  # <-- REMOVED (GE module)
    ...
    def forward(self, x, shape=None, return_intermediates=False):
        ...
        # ===== ResNeXt101 Backbone (Original, Unchanged) =====
        bk_stage1, bk_stage2, bk_stage3, bk_stage4, bk_stage5 = self.bkbone(x)

        # ===== HCO Pathway (Process ResNeXt outputs) =====
        # Stage 1 HCO
        hco_out1 = self.hco_pathway[0](bk_stage1)  # (B, 64, H/4, W/4)

        # Stage 2 HCO: Combine with aligned stage 1 HCO output
        hco1_aligned = self.hco_align[0](hco_out1)  # (B, 256, H/4, W/4)
        hco1_aligned = F.interpolate(hco1_aligned, size=bk_stage2.size()[2:], mode='bilinear', align_corners=True)
        # hco_in2 = bk_stage2 + hco1_aligned
        hco_in2 = self.vheat_ffm[0](hco1_aligned, bk_stage2)
        hco_out2 = self.hco_pathway[1](hco_in2)  # (B, 256, H/8, W/8)

        # Stage 3 HCO
        hco2_aligned = self.hco_align[1](hco_out2)  # (B, 512, H/8, W/8)
        hco2_aligned = F.interpolate(hco2_aligned, size=bk_stage3.size()[2:], mode='bilinear', align_corners=True)
        # hco_in3 = bk_stage3 + hco2_aligned
        hco_in3 = self.vheat_ffm[1](hco2_aligned, bk_stage3)
        hco_out3 = self.hco_pathway[2](hco_in3)  # (B, 512, H/16, W/16)

        # Stage 4 HCO
        hco3_aligned = self.hco_align[2](hco_out3)  # (B, 1024, H/16, W/16)
        hco3_aligned = F.interpolate(hco3_aligned, size=bk_stage4.size()[2:], mode='bilinear', align_corners=True)
        # hco_in4 = bk_stage4 + hco3_aligned
        hco_in4 = self.vheat_ffm[2](hco3_aligned, bk_stage4)
        hco_out4 = self.hco_pathway[3](hco_in4)  # (B, 1024, H/32, W/32)

        # Stage 5 HCO (Final)
        hco4_aligned = self.hco_align[3](hco_out4)  # (B, 2048, H/32, W/32)
        hco4_aligned = F.interpolate(hco4_aligned, size=bk_stage5.size()[2:], mode='bilinear', align_corners=True)
        # hco_in5 = bk_stage5 + hco4_aligned
        hco_in5 = self.vheat_ffm[3](hco4_aligned, bk_stage5)
        hco_out5 = self.hco_pathway[4](hco_in5)  # (B, 2048, H/32, W/32)

        # ===== Replace GE (Pyramid Pooling) with HCO output =====
        fused4 = self.hco_to_64(hco_out5)  # (B, 64, H/32, W/32) - Replaces pyramid_pooling
        ...
        fused2 = F.interpolate(fused2, size=[fused1.size(2) // 2, fused1.size(3) // 2], mode='bilinear',
                               align_corners=True)
